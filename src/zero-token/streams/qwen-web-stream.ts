  ...
  return (model, context, streamOptions) => {
    ...
    const run = async () => {
      try {
        ...
        // Add tool reminder at the END of prompt (both first turn and continuing)
        // This ensures the model sees the instruction right before generating its response
        const toolsAvailable = (context.tools || []).length > 0;
        if (toolsAvailable) {
          if (conversationId) {
            // Continuing turn: send full tool format, not just short reminder
            prompt +=
